...
import com.veterinaria.veterinaria_comoreyes.external.reports.financial.dto.MostUsedPaymentMethodDTO;
import com.veterinaria.veterinaria_comoreyes.external.reports.financial.dto.PaymentMethodDetailDTO;
...
public interface FinancialReportRepository extends JpaRepository<Payment, Long> {
    ...
    // @Query("""
    //             SELECT new com.veterinaria.veterinaria_comoreyes.external.reports.financial.dto.IncomeByHeadquarterDTO(
    //                 h.name,
    //                 SUM(p.amount),
    //                 COUNT(p)
    //             )
    ...
    //             GROUP BY h.name
    //         """)
    // List<IncomeByHeadquarterDTO> getIncomeReportByHeadquarter();

    // @Query("""
    //             SELECT new com.veterinaria.veterinaria_comoreyes.external.reports.financial.dto.MostUsedPaymentMethodDTO(pm.name, COUNT(pm))
    //             FROM Payment p
    //             JOIN p.paymentMethod pm
    //             JOIN p.appointment a
    //             JOIN a.headquarterVetService hvs
    //             JOIN hvs.headquarter h
    //             WHERE p.status = 'COMPLETADA' AND h.name = :headquarterName
    //             GROUP BY pm.name
    //             ORDER BY COUNT(pm) DESC
    //         """)
    // List<MostUsedPaymentMethodDTO> getMostUsedPaymentMethodByHeadquarter(
    //         @Param("headquarterName") String headquarterName);
    ...
    @Query("""
                ...
                GROUP BY h.name
            """)
    List<IncomeByHeadquarterDTO> getIncomeReportByHeadquarterWithinPeriod(
            @Param("startDate") LocalDateTime startDate,
            @Param("endDate") LocalDateTime endDate);

    @Query("""
                SELECT new com.veterinaria.veterinaria_comoreyes.external.reports.financial.dto.PaymentMethodDetailDTO(
                    pm.name, SUM(p.amount), COUNT(p)
                )
                FROM Payment p
                JOIN p.paymentMethod pm
                JOIN p.appointment a
                ...
                JOIN hvs.headquarter h
                WHERE p.status = 'COMPLETADA'
                  AND h.name = :headquarterName
                  AND p.paymentDateTime BETWEEN :startDate AND :endDate
                GROUP BY pm.name
            """)
    List<PaymentMethodDetailDTO> getAllPaymentMethodsByHeadquarterWithinPeriod(
            @Param("headquarterName") String headquarterName,
            @Param("startDate") LocalDateTime startDate,
            @Param("endDate") LocalDateTime endDate);
