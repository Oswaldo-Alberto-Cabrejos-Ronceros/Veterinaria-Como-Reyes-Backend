...
public interface ClientRepository extends JpaRepository<Client, Long> {
    ...
    @Query("""
                SELECT new com.veterinaria.veterinaria_comoreyes.dto.Client.ClientListDTO(
                    c.clientId,
                    c.dni,
                    c.name,
                    c.lastName,
                    c.headquarter.name,
                    CASE WHEN c.status = true THEN 'Activo' ELSE 'Bloqueado' END
                )
                FROM Client c
                WHERE (:dni IS NULL OR c.dni LIKE CONCAT(:dni, '%'))
                  AND (:name IS NULL OR LOWER(c.name) LIKE LOWER(CONCAT(:name, '%')))
                  AND (:lastName IS NULL OR LOWER(c.lastName) LIKE LOWER(CONCAT(:lastName, '%')))
                  AND (:status IS NULL OR c.status = :status)
                  AND (:headquarterId IS NULL OR c.headquarter.headquarterId = :headquarterId)
                ORDER BY c.clientId DESC
            """)
    Page<ClientListDTO> searchClients(
