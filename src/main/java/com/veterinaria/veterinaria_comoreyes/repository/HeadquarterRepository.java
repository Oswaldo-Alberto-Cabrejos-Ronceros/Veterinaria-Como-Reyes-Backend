...
public interface HeadquarterRepository extends JpaRepository<Headquarter, Long> {
   ...
   @Query(value = """
         SELECT h.headquarter_id as headquarterId, h.name, h.phone, h.address,
                h.email, h.district, h.province
         FROM headquarter h
         WHERE (:name IS NULL OR h.name LIKE %:name%)
           AND (:phone IS NULL OR h.phone LIKE %:phone%)
           AND (:address IS NULL OR h.address LIKE %:address%)
           AND (:email IS NULL OR h.email LIKE %:email%)
           AND (:district IS NULL OR h.district LIKE %:district%)
           AND (:province IS NULL OR h.province LIKE %:province%)
         """, countQuery = """
         SELECT COUNT(*)
         FROM headquarter h
         WHERE (:name IS NULL OR h.name LIKE %:name%)
